...
var revisarFecha = function (fecha){
    var fechaR = new Date(fechaR);
    fechaR = fechaR.toLocaleDateString();
    var fechaD = fecha.split("/");
    var fechaH = new Date(Date.now().toLocaleDateString().split("/"));
    if(fechaD[0] == fechaH[0]){
        if(fechaH[0] > fechaD[0]){
            return 0;
        }
        else if(fechaH[1] > fechaD[1]){
            return 0;
        }
    }
    return 1;
